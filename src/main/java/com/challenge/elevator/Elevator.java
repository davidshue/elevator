...
public class Elevator extends Thread {
	...
	public synchronized void respond(int level) {
		System.out.println("elevator " + getNumber() + " pushed " + level + " level");
	...
	public int getPriority(CallRequest callRequest) {
		...
		if (this.status.getDirection() != null) {
			...
			if (factor == 1) {
				score += callRequest.getDirection().getFactor() * diff * 100;
			}
			else {
	...
	public String toString() {
		return "elevator " + getNumber() + " " + getStatus() + " -> level " + getCurrentLevel();
	}
}
